...
TEST_CASE("Create relation, role with <= 255 unicode characters", "[osmchange][node][xml]") {
  for (int i = 1; i <= 255; i++) {
    auto v = repeat("😎", i);
    REQUIRE_NOTHROW(process_testmsg(
      fmt::format(
...
TEST_CASE("Invalid data", "[osmchange][relation][xml]") {
...
TEST_CASE("Very large XML message", "[osmchange][relation][xml]") {
